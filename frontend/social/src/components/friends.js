...
const socket = io.connect('www.mahmoudnabil.tech:3001', {
    auth: {
...
const Chat = () => {
    ...
    const getMyAccount = async () => {
        try {
            const response = await axios.get('www.mahmoudnabil.tech:3001/users/me', {
                headers: {
    ...
    const getChat = async (friendId) => {
        try {
            const response = await axios.get(`www.mahmoudnabil.tech:3001/chat/${friendId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
